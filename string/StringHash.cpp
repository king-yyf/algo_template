...
int main() {
	string s1 = "code", s2 = "forces", s3 = s1 + s2;
	string_hash f1(s1), f2(s2), f3(s3);

	auto h1 = f1.complete_hash();
	auto h2 = f2.complete_hash();
	auto h3 = f3.complete_hash();

	cout << "hash of s1 : " << h1 << "\n";
	cout << "hash of s2 : " << h2 << "\n";
	cout << "hash of s3 : " << h3 << "\n";

	auto hash = concat_hashes(h1, h2, s2.size());

	cout << "check hash of s3 equal (s1 + s2): " << (hash == h3 ? "YES" : "NO") << "\n";

	string s = "codeedforces";  // s[2,5] = "deed"
	string_hash f(s);

	cout << s.substr(2, 4) << ": ";
	cout << (f.is_palindrome(2, 6) ? "is " : "is not ") << "a palindrome\n";
