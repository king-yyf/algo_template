...
模板代码基于 [nealwu rmqlca](https://github.com/nealwu/competitive-programming/blob/master/rmq_lca/rmq_lca.cc), 该模板适用于无权图
...
struct LCA {
    ...
    vector<int> heavy_root, heavy_root_dep, heavy_root_fa; // 后两个 vector 仅用于优化 get_kth_anc
    RMQ<int> rmq;
    bool built = false;

    LCA(int n) : n(n), g(n), fa(n, -1), dep(n), siz(n), dfn(n), ent(n), out(n), tour_ls(n), heavy_root(n){}
    LCA(const vector<vector<int>> &_g) : LCA(int(_g.size())){g = _g;}
    ...
    void build(int root = -1) {
        ...
        euler.clear();
        euler.reserve(2 * n);
        function<void(int, int)> dfs1 = [&](int u, bool hv) {
            heavy_root[u] = hv ? heavy_root[fa[u]] : u;
        ...
        euler_dep.clear();
        heavy_root_dep.resize(n); heavy_root_fa.resize(n);

        for (int i = 0; i < n; i++) {
            heavy_root_dep[i] = dep[heavy_root[i]];
            heavy_root_fa[i] = fa[heavy_root[i]];
        }
        rev_tour_ls = tour_ls;
    ...
    int get_kth_node_on_path(int a, int b, int k) const {
        ...
        if (k < ls)
            return get_kth_anc(a, k);
        else
            return get_kth_anc(b, ls + rs - k);
    }
...
```c++
int anc = g.get_lca(x, y);
```

4. 获取树的直径, 返回<直径长度，{端点u, 端点v}>

```c++
auto diam = g.get_diameter();
```
